...
def count_deadends(rows=20, columns=20, tries=1000, algorithms=None):
    ...
    sorted_algorithms = sorted(algorithms, key=lambda alg: -averages[alg.__name__])
...
def execution_time_generation(rows=20, columns=20, tries=100,
...
def execution_time_resolution(rows=20, columns=20, tries=100,
    ...
    for algo in maze_solvers:
        ...
        # Calcola le medie per il risolutore corrente e le memorizza nel dizionario
        avg_time = float('inf')
        if execution_times:
    ...
    for algo in sorted_solvers:
        ...
        if metrics["average_time"] == float('inf'):
...
def longest_path_length(rows=20, columns=20, tries=100,
                        algorithms=None, show_every_try=False):
    ...
    for algo in algorithms:
        ...
        # Resetta liste per ogni algoritmo
        execution_times = []
        path_lengths = []
        ...
        # Calcola le medie per il generatore corrente e le memorizza nel dizionario
        avg_time = float('inf')
        if execution_times: # Controlla se esistono i tempi
            avg_time = sum(execution_times) / len(execution_times)

        avg_length = 0
        if path_lengths: # Controlla se esistono le lunghezze
            avg_length = sum(path_lengths) / len(path_lengths)
...
def write_on_file(filepath, rows=100, cols=100, tries=100, show_every_try=False):
    ...
    try:
        with open(filepath, "w") as f:
            ...
            #execution_time_generation(rows,cols,tries,gen,show_every_try=show_every_try)
            #print("\n\n")
            #execution_time_resolution(rows, cols, tries, maze_generator=BinaryTree, show_every_try=show_every_try)
            #execution_time_resolution(rows, cols, tries, maze_generator=Sidewinder, show_every_try=show_every_try)
            #execution_time_resolution(rows, cols, tries, maze_generator=RecursiveBacktracker, show_every_try=show_every_try)
            #execution_time_resolution(rows, cols, tries, maze_generator=AldousBroder, show_every_try=show_every_try)
